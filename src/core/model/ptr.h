...
{
//	NS_LOG_ERROR("(PeekPointer (p) =  "<< (PeekPointer (p));
///	printf("(PeekPointer (p) = %d \n", (PeekPointer (p));
//	std::cout <<"(PeekPointer (p)" << (PeekPointer (p) <<std::endl;
	T2 *temp2 = PeekPointer (p);
	T1 *temp1 = dynamic_cast<T1 *>(temp2);
	Ptr<T1> ptr(temp1);
	return ptr;
