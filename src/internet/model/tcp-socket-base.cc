...
Ptr<OutputStreamWrapper> logTcpSocketBase;
Ptr<OutputStreamWrapper> inflightbbr1;
Ptr<OutputStreamWrapper> rttbbr1;
Time m_tmp_rtt = Time::Max ();
uint32_t  m_tmp_bytesInFlight = 0;
...
//                   MakeEnumChecker (CLOSED, "Closed"))
    ...
    .AddTraceSource ("CongestionWindow",
                     ...
                     MakeTraceSourceAccessor (&TcpSocketBase::m_cWndTrace),  // m_tcb->m_cWnd    m_cWndTrace
                     "ns3::TracedValueCallback::Uint32")
  ...
  : TcpSocket ()
{
  NS_LOG_FUNCTION (this);
	...
	std::string rttTrFileName = "bbr-rtt1";
	rttbbr1 = asciirtt.CreateFileStream (rttTrFileName.c_str ());

  	AsciiTraceHelper asciiInflight;
  	std::string inflightTrFileName = "bbr-inflight1";
  	inflightbbr1 = asciiInflight.CreateFileStream (inflightTrFileName.c_str ());

	AsciiTraceHelper asciilogTcpSocketBase;
	std::string tcpsocketbaseName = "tcpsocketbase-log";
	logTcpSocketBase = asciilogTcpSocketBase.CreateFileStream (tcpsocketbaseName.c_str ());
	*logTcpSocketBase->GetStream () << "TcpSocketBase log info :" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetNode" << std::endl;
  m_node = node;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetTcp" << std::endl;
  m_tcp = tcp;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetRtt" << std::endl;
  m_rtt = rtt;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetState" << std::endl;
  return m_state;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetErrno" << std::endl;
  return m_errno;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetSocketType" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetNode" << std::endl;
  return m_node;
}
...
{
  NS_LOG_FUNCTION (this);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Bind" << std::endl;
...
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "TcpSocketBase::Bind (const Address &address)" << std::endl;
...
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetInitialSSThresh" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetInitialSSThresh" << std::endl;
...
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetInitialCwnd" << std::endl;
...
{
//	  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetInitialCwnd" << std::endl;
...
{
  NS_LOG_FUNCTION (this << address);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Connect" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Listen" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Close" << std::endl;
  /// \internal
...
{
  NS_LOG_FUNCTION (this);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ShutdownSend" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ShutdownRecv" << std::endl;
  m_shutdownRecv = true;
  return 0;
}
...
TcpSocketBase::Send (Ptr<Packet> p, uint32_t flags)
{
 ...
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Send" << std::endl;
...
{
  NS_UNUSED (address);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SendTo" << std::endl;
...
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Recv" << std::endl;
...
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "RecvFrom" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetTxAvailable" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetRxAvailable" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetSockName" << std::endl;
  if (m_endPoint != nullptr)
    {
...
{
  NS_LOG_FUNCTION (this << address);
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetPeerName" << std::endl;
...
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "BindToNetDevice" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetupCallback" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DoConnect" << std::endl;
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DoClose" << std::endl;
  switch (m_state)
    {
    case SYN_RCVD:
    case ESTABLISHED:
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "CloseAndNotify" << std::endl;
  if (!m_closeNotified)
    {
      NotifyNormalClose ();
...
{
//	  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "OutOfRange" << std::endl;
...
{
//	  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ForwardUp" << std::endl;
...
{
//	 *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ForwardUp6" << std::endl;
  NS_LOG_LOGIC ("Socket " << this << " forward up " <<
...
TcpSocketBase::ForwardIcmp (Ipv4Address icmpSource, uint8_t icmpTtl,
                            ...
                            uint32_t icmpInfo)
{
//	 *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ForwardIcmp" << std::endl;
...
TcpSocketBase::ForwardIcmp6 (Ipv6Address icmpSource, uint8_t icmpTtl,
                             ...
                             uint32_t icmpInfo)
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ForwardIcmp6" << std::endl;
  NS_LOG_FUNCTION (this << icmpSource << static_cast<uint32_t> (icmpTtl) <<
                   static_cast<uint32_t> (icmpType) <<
                   static_cast<uint32_t> (icmpCode) << icmpInfo);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "IsValidTcpSegment" << std::endl;
...
{
	//*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DoForwardUp" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessEstablished" << std::endl;
  ...
  if (tcpflags == TcpHeader::ACK)
    {
      ...
      if (tcpHeader.GetAckNumber () < m_txBuffer->HeadSequence ())
      ...
      else if (tcpHeader.GetAckNumber () > m_tcb->m_highTxMark)
        {
          ...
          // Receiver sets ECE flags when it receives a packet with CE bit on or sender hasn閳ユ獿responded to ECN echo sent by receiver
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "IsTcpOptionEnabled" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ReadOptions" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "EnterRecovery" << std::endl;
  ...
  //yqy bbr涓病鏈夋洿鏂扮獥鍙?鍙互鑰冭檻姝ゅ娉ㄩ噴鎿 EnterRecovery
...
{
  NS_LOG_FUNCTION (this);
//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DupAck" << std::endl;
  ...
  if (m_tcb->m_congState == TcpSocketState::CA_RECOVERY)
    {
      ...
      //yqy bbr涓病鏈夋洿鏂扮獥鍙?鍙互鑰冭檻姝ゅ娉ㄩ噴鎿 DoRecovery
...
{
//	 *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ReceivedAck" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
  ...
  //yqy
  std::cout << "scoreboardUpdated = " << scoreboardUpdated << std::endl;
  if (scoreboardUpdated)
    {
      std::vector<SequenceNumber32> sackedList = m_txBuffer->GetLastSackedList ();
   //   *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "m_sackEnabled = " << m_sackEnabled <<std::endl;
      for(std::vector<SequenceNumber32>::iterator it = sackedList.begin(); it != sackedList.end(); ++it)
        {
    	  NS_LOG_ERROR("UpdateRateSample " << Simulator::Now ().GetSeconds ());
    	//  *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateRateSample (*it) = " <<std::endl;
          UpdateRateSample (*it);
        }
    }
  ...
  m_tcb->m_rs.m_packetLoss = m_txBuffer->GetLostBytes (1);
...
{
//	 *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessAck" << std::endl;
  ...
  //yqy
 // isDupack = scoreboardUpdated && (!m_sackEnabled && ackNumber == m_txBuffer->HeadSequence () && ackNumber < m_tcb->m_nextTxSequence);
  //*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "isDupack" << isDupack <<std::endl;
...
//yqy
      SequenceNumber32 ackedSeqTmp = m_txBuffer->HeadSequence ();
      NS_LOG_ERROR("ackedSeqTmp = "<<  ackedSeqTmp);
      NS_LOG_ERROR("ackNumber ="<< ackNumber);
      NS_LOG_ERROR("oldHeadSequence" << oldHeadSequence);

      SequenceNumber32 ackedSeq = oldHeadSequence;
      for (; ackedSeq < ackNumber; ackedSeq++)
        {
      ...
      // of RecoveryPoint.
      else if (ackNumber < m_recover && m_tcb->m_congState == TcpSocketState::CA_LOSS)
        {
    	  //yqy
    	  BytesInFlight ();
    	  std::cout << "ProcessAck()  TcpSocketState::CA_LOSS   m_tcb->m_cWnd = " << m_tcb->m_cWnd << std::endl;
      ...
      else
        {
          ...
          if (exitedFastRecovery)
            {
        	  //bbr涓互涓嬩唬鐮佹病鏈夋墽琿
        	  NS_LOG_ERROR("exitedFastRecovery " << exitedFastRecovery);
              NewAck (ackNumber, true);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessListen" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpgradeToMeta" << std::endl;
...
//yqy   no use
void
TcpSocketBase::addcwndrecord()
{
	  MpTcpSocketBase* meta = new (this) MpTcpSocketBase(*this);
	  meta->AddSubflowCwndRecord();
}

/*
濞屸剝婀侀柅姘崇箖鏉╂柨娲栭崐鐓庡箵閸嬫氨娴夋惔鏃傛畱婢跺嫮鎮婇敍灞惧妳鐟欏濞囬悽銊ㄧ箹娑擃亜鍤遍弫鐗堟閿涘苯濮涢懗钘夎嫙娑撳秴鐣崰
 * */
int
TcpSocketBase::ProcessTcpOptions(const TcpHeader& header)
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessTcpOptions" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessSynSent" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
  Ptr<const TcpOptionMpTcpJoin> join;
...
TcpSocketBase::ProcessSynRcvd (Ptr<Packet> packet, const TcpHeader& tcpHeader,
                               const Address& fromAddress, const Address& toAddress)
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessSynRcvd" << std::endl;
  NS_UNUSED (toAddress);
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessWait" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessClosing" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessLastAck" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "PeerClose" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DoPeerClose" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Destroy" << std::endl;
  NS_LOG_FUNCTION (this);
  m_endPoint = nullptr;
  if (m_tcp != nullptr)
    {
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Destroy6" << std::endl;
  NS_LOG_FUNCTION (this);
  m_endPoint6 = nullptr;
  if (m_tcp != nullptr)
    {
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SendEmptyPacket" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SendRST" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DeallocateEndPoint" << std::endl;
  if (m_endPoint != nullptr)
    {
      CancelAllTimers ();
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetupEndpoint" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetupEndpoint6" << std::endl;
  NS_LOG_FUNCTION (this);
...
TcpSocketBase::CompleteFork (Ptr<Packet> p, const TcpHeader& h,
                             const Address& fromAddress, const Address& toAddress)
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "CompleteFork" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ConnectionSucceeded" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddSocketTags" << std::endl;
  /*
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SendDataPacket" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SendPacket" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateRttHistory" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SendPendingData" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "BytesInFlight" << std::endl;
 // uint32_t bytesInFlight = m_txBuffer->BytesInFlight ();
      ...
      if (retransOut > m_dupAckCount)
        {
    	  ...
    	  //yqy
    	  duplicatedSize = retransOut - m_dupAckCount;
      ...
      else
        {
          ...
          //yqy
    	  duplicatedSize = m_dupAckCount - retransOut;
  ...
  //yqy
  if (m_tcb->m_bytesInFlight != bytesInFlight)
    {
      m_tcb->m_priorInFlight = m_tcb->m_bytesInFlight.Get ();
  ...
  m_tcb->m_bytesInFlight_bbr = bytesInFlight;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Window" << std::endl;
  return std::min (m_rWnd.Get (), m_tcb->m_cWnd.Get ());
}

//yqy
uint32_t
TcpSocketBase::getSfWindow (void) const
{
  //return std::min (m_rWnd.Get (), m_tcb->m_cWnd.Get ());
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AvailableWindow" << std::endl;
  uint32_t win = Window ();             // Number of bytes allowed to be outstanding
  uint32_t inflight = BytesInFlight (); // Number of outstanding bytes    鏈畬鎴愮殑瀛楄妭鏁
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AdvertisedWindowSize" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ReceivedData" << std::endl;
  NS_LOG_FUNCTION (this << tcpHeader);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "EstimateRtt" << std::endl;
  ...
  if (!m.IsZero ())
    {
      ...
      //yqy
      m_tcb->m_lastRtt_bbr = m_tcb->m_lastRtt.Get ();
...
{
//*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "NewAck" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ReTxTimeout" << std::endl;
  NS_LOG_FUNCTION (this);
  ...
  NS_ASSERT_MSG (BytesInFlight () <= m_tcb->m_segmentSize, "In flight (" << BytesInFlight () <<") there is more than one segment (" << m_tcb->m_segmentSize << ")");
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DelAckTimeout" << std::endl;
  m_delAckCount = 0;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "LastAckTimeout" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "PersistTimeout" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "DoRetransmit" << std::endl;
  NS_LOG_FUNCTION (this);
  bool res;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "CancelAllTimers" << std::endl;
  m_retxEvent.Cancel ();
  m_persistEvent.Cancel ();
  m_delAckEvent.Cancel ();
  m_lastAckEvent.Cancel ();
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "TimeWait" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetSndBufSize" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetSndBufSize" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetRcvBufSize" << std::endl;
  NS_LOG_FUNCTION (this << size);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetRcvBufSize" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetSegSize" << std::endl;
  NS_LOG_FUNCTION (this << size);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetSegSize" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetConnTimeout" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetConnTimeout" << std::endl;
  return m_cnTimeout;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetSynRetries" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetSynRetries" << std::endl;
  return m_synRetries;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetDataRetries" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetDataRetries" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetDelAckTimeout" << std::endl;
  NS_LOG_FUNCTION (this << timeout);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetDelAckTimeout" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetDelAckMaxCount" << std::endl;
  NS_LOG_FUNCTION (this << count);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetDelAckMaxCount" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetTcpNoDelay" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetTcpNoDelay" << std::endl;
  return m_noDelay;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetPersistTimeout" << std::endl;
  NS_LOG_FUNCTION (this << timeout);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetPersistTimeout" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetAllowBroadcast" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetAllowBroadcast" << std::endl;
  return false;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GenerateUniqueMpTcpKey" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddMpTcpOptions" << std::endl;
  NS_LOG_FUNCTION(this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddOptions" << std::endl;
  NS_LOG_FUNCTION (this << header);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessOptionMpTcp" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessOptionWScale" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "CalculateWScale" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddOptionWScale" << std::endl;
  NS_LOG_FUNCTION (this << header);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessOptionSack" << std::endl;
  NS_LOG_FUNCTION (this << option);

  const TcpOptionSack *s;
  s = (const TcpOptionSack *) (PeekPointer (option));
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessOptionSackPermitted" << std::endl;
  NS_LOG_FUNCTION (this << option);
...
{
//*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddOptionSackPermitted" << std::endl;
  NS_LOG_FUNCTION (this << header);
  NS_ASSERT (header.GetFlags () & TcpHeader::SYN);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddOptionSack" << std::endl;
  NS_LOG_FUNCTION (this << header);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "ProcessOptionTimestamp" << std::endl;
  NS_LOG_FUNCTION (this << option);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddOptionTimestamp" << std::endl;
  NS_LOG_FUNCTION (this << header);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateWindowSize" << std::endl;
  NS_LOG_FUNCTION (this << header);
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetMinRto" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetMinRto" << std::endl;
  return m_minRto;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetClockGranularity" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetClockGranularity" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetTxBuffer" << std::endl;
  return m_txBuffer;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetRxBuffer" << std::endl;
  return m_rxBuffer;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetRetxThresh" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateCwnd" << std::endl;
...
{
	//*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateCwndInfl" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateSsThresh" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateCongState" << std::endl;
...
{
//	 *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateEcnState" << std::endl;
...
{
//	 *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateNextTxSequence" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateHighTxMark" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateBytesInFlight" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateRtt" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetCongestionControlAlgorithm" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetRecoveryAlgorithm" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "Fork" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SafeSubtraction" << std::endl;
  if (a > b)
    {
      return a-b;
    }
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "NotifyPacingPerformed" << std::endl;
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetEcn" << std::endl;
  NS_LOG_FUNCTION (this);
  m_ecnMode = ecnMode;
}
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "TcpCongControl" << std::endl;
  if (m_congestionControl->HasCongControl ())
    {
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdatePacketSent" << std::endl;
	NS_LOG_ERROR("UpdatePacketSent "<< "seq = "<<seq <<" "<<"sz ="<< sz);
  NS_LOG_FUNCTION (this << seq << sz);
  Time now = Simulator::Now ();
  if (m_tcb->m_bytesInFlight_bbr == 0)
    {
...
{
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateRateSample" << std::endl;

//	NS_LOG_ERROR("UpdateRateSample" << "seq ="<< seq );
  if (m_pps.find(seq) == m_pps.end())
    {
	 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "m_pps.find(seq) == m_pps.end()" << std::endl;
      return;
    }
  ...
  if (item.m_deliveredTime == Time::Max ())
    {
	 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "item.m_deliveredTime" << item.m_deliveredTime <<std::endl;
      return;
    }
 ...
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "item.m_delivered" << item.m_delivered <<std::endl;
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "m_tcb->m_rs.m_priorDelivered" << m_tcb->m_rs.m_priorDelivered <<std::endl;
...
{
	//NS_LOG_ERROR("GenerateRateSample" );
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "GenerateRateSample" << std::endl;
  NS_LOG_FUNCTION (this);
 ...
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "m_tcb->m_rs.m_priorTime = " << m_tcb->m_rs.m_priorTime << std::endl;
 ...
 // *logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "m_tcb->m_rs.m_interval" << m_tcb->m_rs.m_interval << std::endl;
  std::cout <<"m_tcb->m_rs.m_interval.GetSeconds () =  " << m_tcb->m_rs.m_interval.GetSeconds () << std::endl;
...
{
//	NS_LOG_ERROR("OnApplicationWrite" );
//	*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "OnApplicationWrite" << std::endl;
  if ((m_txBuffer->TailSequence () - m_tcb->m_nextTxSequence) < (int) m_tcb->m_segmentSize &&
//       m_tcb->m_pendingTransmissions == 0 &&
       m_tcb->m_bytesInFlight_bbr < m_tcb->m_cWnd)//   &&
//       GetLostCount () <= GetRetransmitsCount ())
    {
	  //修改：认为在m_bytesInFlight_bbr = 0时，m_appLimited应该为m_delivered；在m_bytesInFlight_bbr != 0时，m_appLimited应该为m_delivered + 1.
      m_tcb->m_appLimited = m_tcb->m_delivered + m_tcb->m_bytesInFlight_bbr ?  : 1;
    }
}
...
RttHistory::RttHistory (SequenceNumber32 s, uint32_t c, Time t)
  : seq (s),
    ...
    retx (false)
{
	//*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "RttHistory" << std::endl;
}
...
RttHistory::RttHistory (const RttHistory& h)
  : seq (h.seq),
    ...
    retx (h.retx)
{
	//*logTcpSocketBase->GetStream () << Simulator::Now ().GetSeconds () << " " << "RttHistory::RttHistory (const RttHistory& h)" << std::endl;
}
