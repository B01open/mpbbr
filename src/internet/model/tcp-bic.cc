    ...
    m_b (sock.m_b)
{
	...
	std::string cwndTrFileName = "bic-cwnd";
	cwndStream = ascii.CreateFileStream (cwndTrFileName.c_str ());
	//*cwndStream->GetStream () << 0 << " " << 10 << std::endl;
  ...
  if (tcb->m_cWnd < tcb->m_ssThresh)
    {
      ...
      *cwndStream->GetStream () << Simulator::Now ().GetSeconds () << "  " << tcb->m_cWnd/1.0/tcb->m_segmentSize <<"  "<< tcb->m_lastRtt.Get ().GetSeconds () <<std::endl;
    }
...
TcpBic::Update (Ptr<TcpSocketState> tcb)
{
  ...
  std::cout << Simulator::Now ().GetSeconds () << "  Update   " <<  std::endl;
  m_lastCwnd = segCwnd;
  *cwndStream->GetStream () << Simulator::Now ().GetSeconds () << "  " << tcb->m_cWnd/1.0/tcb->m_segmentSize <<"  "<< tcb->m_lastRtt.Get ().GetSeconds () <<std::endl;
