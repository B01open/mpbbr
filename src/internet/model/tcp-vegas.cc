...
Ptr<OutputStreamWrapper> cwndvegas;
    ...
    m_begSndNxt (0)
{
  NS_LOG_FUNCTION (this);
	...
	std::string cwndTrFileName = "vegas-cwnd";
	cwndvegas = ascii.CreateFileStream (cwndTrFileName.c_str ());
	//*cwndvegas->GetStream () << 0 << " " << 10 << std::endl;
}
                     ...
                     const Time& rtt)
{
  ...
  *cwndvegas->GetStream () << Simulator::Now ().GetSeconds () << "   " << tcb->m_cWnd/1.0/tcb->m_segmentSize << "   "<< tcb->m_lastRtt.Get ().GetSeconds ()<< std::endl;
