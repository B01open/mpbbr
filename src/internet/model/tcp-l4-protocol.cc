...
//CreateSocket在此处调用两次，一次是发送端设置，一次是接收端设置；之后的关于mptcp设置的socket没有起作用
Ptr<Socket>
TcpL4Protocol::CreateSocket (TypeId congestionTypeId, TypeId recoveryTypeId)
{
  ...
  //yqy  add different tcp congestion algo
  Ptr<TcpCongestionOps> algo1;
  if(socketId == 0){
	  std::cout<< "socketId0 = " << socketId << std::endl;
	  ObjectFactory congestionAlgorithmFactory;
	  congestionAlgorithmFactory.SetTypeId (TcpBbr::GetTypeId ());  //TypeIdValue (TcpBbr::GetTypeId ()); TcpNewReno
	  algo1 = congestionAlgorithmFactory.Create<TcpCongestionOps> ();
	  socketId++;
  }else if(socketId == 1) {
	  std::cout<< "socketId1 = " << socketId << std::endl;
	  ObjectFactory congestionAlgorithmFactory;
	  congestionAlgorithmFactory.SetTypeId (TcpBbr::GetTypeId ());  //TypeIdValue (TcpBic::GetTypeId ()); TcpNewReno
	  algo1 = congestionAlgorithmFactory.Create<TcpCongestionOps> ();
	  socketId++;
  } else if(socketId == 2){
	  std::cout<< "socketId2 = " << socketId << std::endl;
	  ObjectFactory congestionAlgorithmFactory;
	  congestionAlgorithmFactory.SetTypeId (TcpVegas::GetTypeId ());  //TypeIdValue (TcpVegas::GetTypeId ()); TcpNewReno
	  algo1 = congestionAlgorithmFactory.Create<TcpCongestionOps> ();
	  socketId++;
  } else {
	  std::cout<< "socketId3 = " << socketId << std::endl;
	  ObjectFactory congestionAlgorithmFactory;
	  congestionAlgorithmFactory.SetTypeId (TcpNewReno::GetTypeId ());  //TypeIdValue (TcpNewReno::GetTypeId ()); TcpNewReno
	  algo1 = congestionAlgorithmFactory.Create<TcpCongestionOps> ();
	  socketId++;
  }
  socket->SetCongestionControlAlgorithm (algo1);
*/
