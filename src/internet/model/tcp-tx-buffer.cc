...
Ptr<OutputStreamWrapper> logTcpTxBuffer;
  ...
  : m_maxBuffer (32768), m_size (0), m_sentSize (0), m_firstByteSeq (n)
{
	...
	AsciiTraceHelper TcpTxBufferLog;
	std::string TcpTxBufferName = "TcpTxBuffer-log";
	logTcpTxBuffer = TcpTxBufferLog.CreateFileStream (TcpTxBufferName.c_str ());
	*logTcpTxBuffer->GetStream () << "TcpTxBuffer log info :" << std::endl;
}
...
{
 // *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "HeadSequence" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "TailSequence" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "Size" << std::endl;
  return m_size;
}
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "MaxBufferSize" << std::endl;
  return m_maxBuffer;
}
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetMaxBufferSize" << std::endl;
  m_maxBuffer = n;
}
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "Available" << std::endl;
...
TcpTxBuffer::SetHeadSequence (const SequenceNumber32& seq)
{
 ...
 // *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetHeadSequence" << std::endl;
  m_firstByteSeq = seq;
...
{
//	 *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "Add" << std::endl;
...
{
//	 *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "SizeFromSequence" << std::endl;
  NS_LOG_FUNCTION (this << seq);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "CopyFromSequence" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetNewSegment" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetTransmittedSegment" << std::endl;
  NS_LOG_FUNCTION (this << numBytes << seq);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "FindHighestSacked" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "SplitItems" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "GetPacketFromList" << std::endl;
  NS_LOG_FUNCTION (this << numBytes << seq);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "MergeItems" << std::endl;
  NS_ASSERT (t1 != nullptr && t2 != nullptr);
  NS_LOG_FUNCTION (this << *t1 << *t2);
  NS_LOG_INFO ("Merging " << *t2 << " into " << *t1);

  //yqy
 // NS_ASSERT_MSG (AreEquals (t1->m_sacked, t2->m_sacked), "Merging one sacked and another not sacked. Impossible");
 // NS_ASSERT_MSG (AreEquals (t1->m_lost, t2->m_lost),"Merging one lost and another not lost. Impossible");
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "RemoveFromCounts" << std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "DiscardUpTo" << std::endl;
  NS_LOG_FUNCTION (this << seq);
...
{
	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "Update" << std::endl;
	NS_LOG_ERROR("update " << Simulator::Now ().GetSeconds ());
  NS_LOG_FUNCTION (this);
  ...
  NS_LOG_ERROR("update " << Simulator::Now ().GetSeconds () << "list.size () = " << list.size());
  ...
  for (auto option_it = list.begin (); option_it != list.end (); ++option_it)
    {
      ...
      SequenceNumber32 beginOfCurrentPacket = m_firstByteSeq;
      NS_LOG_ERROR("m_firstByteSeq " << m_firstByteSeq);
      NS_LOG_ERROR("m_sentSize " << m_sentSize);
      NS_LOG_ERROR("(*option_it).first " << (*option_it).first);
     // *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "m_firstByteSeq + m_sentSize " << (m_firstByteSeq + m_sentSize) <<std::endl;
      *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "(*option_it).first " << (*option_it).first <<std::endl;
      ...
      while (item_it != m_sentList.end ())
        {
          ...
          *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "beginOfCurrentPacket " << beginOfCurrentPacket <<std::endl;
          *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "(*option_it).first " << (*option_it).first <<std::endl;

          *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "beginOfCurrentPacket + pktSize " << beginOfCurrentPacket + pktSize <<std::endl;
          *logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "(*option_it).second " << (*option_it).second <<std::endl;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "UpdateLostCount" << std::endl;
  NS_LOG_FUNCTION (this);
  uint32_t sacked = 0;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "IsLost" << std::endl;
  NS_LOG_FUNCTION (this << seq);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "NextSeg" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "BytesInFlight" << std::endl;
  NS_ASSERT_MSG (m_sackedOut + m_lostOut <= m_sentSize,  "Count of sacked " << m_sackedOut << " and lost " << m_lostOut <<
                 " is out of sync with sent list size " << m_sentSize <<" " << *this);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "BytesInFlightRFC" << std::endl;
  PacketList::const_iterator it;
  TcpTxItem *item;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "IsLostRFC" << std::endl;
  NS_LOG_FUNCTION (this << seq);
  uint32_t count = 0;
  uint32_t bytes = 0;
  PacketList::const_iterator it;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "ResetRenoSack" << std::endl;
  NS_LOG_FUNCTION (this);
...
//yqy       ResetScoreboard == ResetRenoSack
void
TcpTxBuffer::ResetScoreboard ()
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "ResetScoreboard" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "ResetSentList" << std::endl;
  NS_LOG_FUNCTION (this);
  TcpTxItem *item;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "ResetLastSegmentSent" << std::endl;
  NS_LOG_FUNCTION (this);
  if (!m_sentList.empty ())
    {
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "SetSentListLost" << std::endl;
  NS_LOG_FUNCTION (this);
  m_retrans = 0;
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "IsHeadRetransmitted" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "DeleteRetransmittedFlagFromHead" << std::endl;
  NS_LOG_FUNCTION (this);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "MarkHeadAsLost" << std::endl;
  if (m_sentList.size () > 0)
    {
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "AddRenoSack" << std::endl;
  NS_LOG_FUNCTION (this);
  NS_ASSERT (m_sentList.size () > 1);
...
{
//	*logTcpTxBuffer->GetStream () << Simulator::Now ().GetSeconds () << " " << "ConsistencyCheck" << std::endl;
