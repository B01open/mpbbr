...
main (int argc, char *argv[])
{
 ...
 Config::SetDefault ("ns::MpTcpNdiffPorts::MaxSubflows", UintegerValue (sfNum));
  ...
  NodeContainer n8n5 = NodeContainer(nodes.Get(8),nodes.Get(5));
  ...
  NetDeviceContainer d8d5 =p2pmptx2.Install(n8n5);
  //tchPfifoFastAccess.Install(d3d4);
 ...
 // p2p.SetDeviceAttribute ("ReceiveErrorModel", PointerValue (&error_model));
  ...
  NetDeviceContainer d2d9 =p2prx3.Install(n2n9);

  PointToPointHelper p2prx4;
  p2prx4.SetDeviceAttribute("DataRate",StringValue("20Mbps"));
  p2prx4.SetChannelAttribute("Delay",StringValue("1ms"));
  NetDeviceContainer d6d9 =p2prx4.Install(n6n9);
  ...
  ApplicationContainer clientApps, clientApps2, clientApps3;
  ...
  //client 3
  BulkSendHelper source3 ("ns3::TcpSocketFactory",InetSocketAddress (i2i9.GetAddress (1), port));
  source3.SetAttribute ("MaxBytes", UintegerValue (0));
  source3.SetAttribute ("SendSize", UintegerValue (tcp_adu_size));
  clientApps3 = source3.Install (nodes.Get(8));
