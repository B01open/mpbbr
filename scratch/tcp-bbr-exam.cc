...
static void ChangeDataRate ()
{
  if (m_state)
    ...
    Simulator::Schedule (Seconds (10), &ChangeDataRate);
}
...
//	LogComponentEnable("TcpSocketBase", LOG_ERROR);
	// TCP鐨勬嫢濉炴帶鍒
	 ...
	 // Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue ("ns3::TcpBic"));  //TcpBbr   TcpNewReno  TcpBic
	//  Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (1000));
	  ...
	  //Enable MPTCP 閰嶇疆mptcp澶撮儴锛岃矾寰勭鐞嗘ā寮忥紝鑷祦鏁伴噺
  ...
  if (scenario == "2")
    {
      Simulator::Schedule (Seconds (5), &ChangeDataRate);
    }
...
//  configstore.ConfigureDefaults ();
  ...
  Ptr<PacketSink> sink1 = DynamicCast<PacketSink> (d.GetLeft (0));
