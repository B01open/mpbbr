...
// connecting routers and hosts and assign ip addresses 连接路由和主机，并分配ip
  ...
  NodeContainer h0r1 = NodeContainer (client, router.Get (0));

  NodeContainer h1r0 = NodeContainer (server, router.Get (0));
...
// We create the channels first without any IP addressing information 创建管道及连接
  ...
  NetDeviceContainer l4h1r0 = p2p.Install (h1r0);
...
// Later, we add IP addresses. 分配ip
  ...
  Ipv4InterfaceContainer i4h1r0 = ipv4.Assign (l4h1r0);
...
// Create router nodes, initialize routing database and set up the routing
  ...
  BulkSendHelper source ("ns3::TcpSocketFactory",InetSocketAddress (i4h1r0.GetAddress (0), port));
