...
main (int argc, char *argv[])
{
 ...
 //  LogComponentEnable("MpTcpSocketBase", LOG_ERROR);
  ...
  Config::SetDefault ("ns3::TcpL4Protocol::SocketType", StringValue (transport_prot));   //TcpBbr   TcpBic
...
// connecting routers and hosts and assign ip addresses 连接路由和主机，并分配ip
    ...
    NodeContainer h0r0 = NodeContainer (client, server);
    NodeContainer h0r1 = NodeContainer (client, server);
    NodeContainer h0r2 = NodeContainer (client, server);
...
// We create the channels first without any IP addressing information 创建管道及连接
  ...
  tchPfifoFastAccess.Install(l1h0r1);

  p2p.SetDeviceAttribute ("DataRate", StringValue ("10Mbps"));
  p2p.SetChannelAttribute ("Delay", StringValue ("1ms"));
  p2p.SetDeviceAttribute ("ReceiveErrorModel", PointerValue (&error_model));
  NetDeviceContainer l2h0r2 = p2p.Install (h0r1);
  tchPfifoFastAccess.Install(l2h0r2);
...
// Create router nodes, initialize routing database and set up the routing
  ...
  BulkSendHelper source ("ns3::TcpSocketFactory",InetSocketAddress (i2r0r2.GetAddress (1), port));
