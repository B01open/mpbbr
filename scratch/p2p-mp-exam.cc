...
// We create the channels first without any IP addressing information 创建管道及连接
  ...
  p2p.SetDeviceAttribute ("DataRate", StringValue (access_bandwidth));
  p2p.SetChannelAttribute ("Delay", StringValue (access_delay));
...
// Create router nodes, initialize routing database and set up the routing
  ...
  // Create OnOff Application  // Create BuldSend Application to send Tcp packets
  BulkSendHelper source ("ns3::TcpSocketFactory",InetSocketAddress (i0h0r0.GetAddress (1), port));
